...
title: "xxx"
subtitle: "Run xxx"
analyst: "xxx instrumental xxx data"
run date: "mm/dd/yyyy"
output:
  html_document: 
    df_print: paged
...
\*\*\*Please add to publications "We acknowledge the analytical
contributions of the CU Boulder Earth Systems Stable Isotope Lab
(CUBES-SIL) Core Facility (RRID:SCR_019300)"</b>
...
<li><i>Sampling and prep of your materials are unique to your sample
set</i></li>

<li><b>Lab:</b> University of Colorado Boulder Earth Systems Stable
Isotope Laboratory (CUBES--SIL)</li>

<li><b>Preparation:</b> Weighed samples and a suite of standards in 12
ml Exetainer Sample Vials (Labco) were flushed with helium for 10 min.
Heated vials were acidified with 5-10 drops of phosphoric acids (70
degrees C)</li>

<li><b>Instrumentation:</b> Thermo Delta V continuous-flow stable
isotope ratio mass spectrometer attached to a gas bench device</li>

<li><b>Data correction and calculations software:</b> In-house R scripts
using R Statistical Software (v4.2.0; R Core Team 2022) and the RStudio
2022.07.0 interface (RStudio Team, 2022) with Tidyverse and IsoVerse
packages (Wickham et al. 2023; Kopf et al., 2021). <br>\* R Core Team
(2022). R: A language and environment for statistical computing. R
Foundation for Statistical Computing, Vienna, Austria. URL
<https://www.R-project.org/>. <br>\* RStudio Team (2022). RStudio:
Integrated Development Environment for R. RStudio, PBC, Boston, MA URL
<http://www.rstudio.com/>. <br>\* Wickham H, Vaughan D, Girlich M
(2023). tidyr: Tidy Messy Data. <https://tidyr.tidyverse.org>,
<https://github.com/tidyverse/tidyr>. <br>\* Kopf, S.,
Davidheiser-Kroll, B., and Kocken, I.: Isoreader: An R package to read
stable isotope data files for re- producible research, J. Open Source
Softw., 6, 2878, <https://doi.org/10.21105/joss.02878>, 2021.</li>

<li><b>Notation:</b> Stable isotope ratios are reported in delta (δ)
notation relative to the Vienna Pee Dee Belemnite (VPDB) standard for
carbon and air for nitrogen, where δ = ([(RSample/RStandard) − 1]), R is
the ratio of the heavier mass isotope to the lighter mass isotope, as
per mil (‰)</li>

<li><b>Data correction and calculations:</b> Sample δ13C values in this
run were corrected for XXX, then scale using a 4 point scale correction;
δ18O values of samples were corrected for XXX, then scale using a 3
point scale correction. Errors estimated using the residual standard
error of the linear scale correction model.</li>

</ul>

<u>Run Details:</u> <br>Run went well in terms of chromatography however
standards did not do a good job of creating a robust yeild (weight vs
peak area). This makes it difficult to fully trust that the standards
are reflecting the true linearity, therefore the linearity (and drift)
corrections may not be adequate, espcially for very small samples. Some
samples were very low - low peak areas (noted below), use with caution.
<br>Precision on the scale corrections were +/- XXX permil (d13C,
"lindrift" corr) and +/- XXX permil (d18O, "lindrift" corr). All
analyses (samples = sa, standards = st) culled are listed below (see
output file for details on culling): <br> d18O.error.S = XXX <br>
d13C.error.S = XXX

<u>Sample Details:</u> Problem samples and standards (do not use or use
with caution): <br><b>DO NOT USE</b> <br>\* XXX - CU YULE large d18O
outlier (st) <br>\* XXX - IAEA-603 drift std large d18O outlier (st)

<br><b>USE WITH CAUTION (SMALL)</b> <br>\* XXX - Sample peak area
smaller than linearity bracket (and largest d13C SD of entire run) (sa)
<br>\* XXX - Sample peak area smaller than linearity bracket (largest
d18O SD and 2nd largest d13C SD of entire run) (sa)
...
session<-"XXX"  #update this to name your own session
Run <- "XXX"
...
Standards <- readRDS(url("https://github.com/cubessil/clumpsDRcodes/blob/master/Standards.RDS?raw=true"))
 ...
 corr.std<-"IAEA-603"  #enter your standard names here
 mon.std<-"CU YULE"
 dis.std<-"NBS18"
dis2.std<-"USGS44"
...
data <- 
  raw.data %>% 
  filter(Nr. > 6) %>% #THIS IS NORMALLY=5 (for all 10 peaks) changing to only 9 last peaks due to analysis 24971
...
Identify outliers: Look at standard deviations of the stable isotope
values of the individual peaks (typically there are 10 peaks). Often the
"cutoff" of outliers tends to be sd of 0.075 - 0.1 permil, and coincides
with small peak areas
...
Remove major outliers based on large standard deviation of peaks, then
redo plots - also, adds culled data to "culled data" tab that shows
samples and standards that shouldn't be used
...
Plot yields of all samples and standards, as well as just the standards,
using INTERACTIVE plots. Look for major outliers that coincide with
yield problems. Note that the linear model in the yield.stds figure here
is based on ALL the standard data

Check to see that samples all fall within linearity range, and for any
other obvious outliers: \* do linearity standards cover the area space
of your samples and other standards? \* do all the standards show
typical trends between area and mass? Do you see very general trends
like that in your samples (sample sets with wide ranges in weight
percent carbonate will not show a strong relationship)?
...
Makes plots of individual analysis files for analyses in the culled.data
dataframe - use to check peak sizes, signs of atmospheric peaks, and
...
Evaluate average offset and standard deviation of combined linearity and
drift standards. Does NOT include linearity or drift correction to data
...
Assess effects: Apply offset correction to the whole dataset of raw
values, and check the monitoring standards
...
Assess effects: Apply drift correction to the whole dataset of raw
values, and check the monitoring standards
...
Assess effects: Apply linearity correction to the whole dataset of raw
values, and check the monitoring standards
...
Because a run can be affected by both drift and linearity, here we
assess the affects of apply both the linearity and drift corrections to
the raw data. This uses the linearity corrected values, and looks at the
drift in those values
...
Assess effects: Apply drift correction to the whole dataset of linearity
corrected values, and check the monitoring standards
...
Pick the appropriate column of either raw or corrected data to use in
the scale regression. If there is no observable effects of linearity or
drift (or both), use the raw values. The default is to select the raw
values. Then this correct is applied to the whole dataset, using the
same choice of previously corrected (or raw) values as used in making
the scale correction

```{r apply.scale.corr.C}

# replace the character string here with the correction column you want to use
col_for_scale_C <- "drift"  # options to substitute in here are: "raw" or "offset" or "drift" or  "lin" or  "lindrift"
...
C.scale.all <- 
  ...
  geom_text(
           x = mean(scale.corr$d13C,na.rm=T), 
           y = mean(scale.corr$C.acc,na.rm=T), 
...
Assess effects: Check the monitoring standards after applying the scale
correction
...
Assess effects: Apply offset correction to the whole dataset of
linearity corrected values, and check the monitoring standards
...
Assess effects: Apply linearity correction to the whole dataset of raw,
and check the monitoring standards
...
O.scale.all <- 
  ...
  geom_point(shape=21, fill="blue", size = 2) +
  geom_text(
           x = mean(scale.corr$d18O,na.rm=T), 
           y = mean(scale.corr$O.acc,na.rm=T), 
...
#label as do not use
#label as do not use
data[data$Analysis == XXX, "Do_not_use"] <- "yes"     
data[data$Analysis == XXX, "Do_not_use"] <- "yes"

# label those samples with reason they were culled, in main dataset
data[data$Analysis == XXX, "Why_culled"] <- "large 18O outlier (st)"     
data[data$Analysis == XXX, "Why_culled"] <- "large 18O outlier (st)"
...
culled.data[culled.data$Analysis == XXX, "Why_culled"] <- "large 18O outlier (st)"     
culled.data[culled.data$Analysis == XXX, "Why_culled"] <- "large 18O outlier (st)"
...
keydata <- data %>% ungroup() %>%  filter(!(Identifier1 %in% c(corr.std, mon.std, dis.std, dis2.std))) %>% select(Run,Analysis, Identifier1, mass,area44,d13C.scale,d13C.error.S, d18O.scale,d18O.error.S,PercentCO3, Do_not_use, Why_culled)
