...
effective_label_summary <- growth_data |> 
  ...
  mutate(
    ...
    effective_at13C = F13C.label * 100,
  ) |> 
...
growth_data_summary |> 
  ...
  gt(rowname_col = "well", groupname_col = "depth_str")
...
gt_assim_data_fmol_summary <- assim_data_fmol_summary |> 
  select(-c(well, depth)) |> 
  gt(rowname_col = "site") |> 
  tab_header(title = "Mean Cell-Specific Carbon Assimilation Rates") |> 
  ...
  tab_spanner(columns = everything(), label = "Amendment") |> 
...
gtsave(gt_assim_data_fmol_summary, filename = "data_output/mean_cscar.html")
