...
growth_data |> 
  ...
  ggplot(
    aes(x = mu.d, y = depth_str, color = amendment)
  ) +
...
```{r}
gt_gen_times <- growth_data_summary |> 
  ...
  cols_label(
    ...
    mean_gen.d = "Mean", max_gen.d = "Max", min_gen.d = "Min"
  ) |> 
  tab_spanner(columns = c(max_gen.d,mean_gen.d, min_gen.d), label = "Generation Time (Days)") |> 
  cols_align("center") |> 
  fmt_number(columns = c(mean_gen.d, max_gen.d, min_gen.d), decimals = 0) |> 
  data_color(columns = c(mean_gen.d, max_gen.d, min_gen.d), palette = "viridis",
             reverse = TRUE)
