...
data_path <- "data/nanoSIMS_tables/collected_tables_reanalysis/"


if (file.exists("cache/nanoSIMS_data_reanalysis.RDS")) {
  roi_data <- readRDS("cache/roi_data_reanalysis.RDS")
} else {
...
p_ratio_combined <- cowplot::plot_grid(
  p_ratio,
  p_ratio_zoom,
  ncol = 1,
  labels = c("A", "B")
...
voa_corrected_ctl |>
  ...
  dplyr::rename(Acetate_uM = Acetate, Formate_uM = Formate) |>
  gt() |>
...
voa_concs <- voa_corrected_ctl |>
  ...
  filter(!is_control) |>
  select(sample_id, compound, uM_corr) |>
  dplyr::rename(voa_conc_uM = uM_corr) |>
...
growth_data <- roi_data |>
  ...
  dplyr::rename(coupon_id = sample_id) |>
  ...
  mutate(
    active = if_else(
      F13C - F13C.error >= F13C.activity_threshold,
      true = TRUE,
      false = FALSE
    ),
    mu.d = if_else(
...
effective_label_summary <- growth_data |>
  ...
  mutate(
    well = factor(well, levels = c("BA1B", "BA4A", "BA3A")),
    depth = factor(as.character(depth), levels = c("20", "150", "250", "270"))
  ) |>
  arrange(well, depth)
...
gt_label_summary <- effective_label_summary |>
  ...
  data_color(
    columns = c(
      isotopic_label_acetate,
      isotopic_label_formate,
  ...
  cols_hide(
    columns = c(
      well,
      moles_pre_acetate,
      moles_pre_bicarbonate,
      moles_pre_formate,
      moles_spike_acetate,
      moles_spike_formate,
      moles_spike_bicarbonate,
      moles_total_acetate,
      moles_total_bicarbonate,
      moles_total_formate,
      moles_total_acetate
    )
  ) |>
  tab_style(
    style = cell_borders(color = "black", sides = c("top"), weight = px(3)),
...
p_13F <- growth_data |>
  ...
  geom_pointinterval(
    aes(
      ymin = F13C - F13C.error,
      ymax = F13C + F13C.error
    ),
    size = 0.5
  ) +
  geom_hline(yintercept = neg_ctl_mean, linewidth = 0.5, color = "black") +
  geom_segment(
    aes(
      y = F13C.label,
      yend = F13C.label,
      x = as.numeric(factor(amendment)) - 0.3,
      xend = as.numeric(factor(amendment)) + 0.3
    ),
    linewidth = 0.5,
    color = "blue"
  ) +
  annotate(
    geom = "rect",
    ymin = neg_ctl_mean - (3 * neg_ctl_sd),
    ymax = neg_ctl_mean + (3 * neg_ctl_sd),
    xmin = -Inf,
    xmax = Inf,
    color = NA,
    fill = "red",
  ...
  labs(
    y = latex2exp::TeX("$^{13}F_{biomass}$"),
    x = latex2exp::TeX("Ranked $^{13}F_{biomass}$"),
    color = "Quantified"
  ) +
  theme_bw() +
  theme(
...
p_13F_zoom <- p_13F %+%
  coord_cartesian(ylim = c(0.008, 0.015), xlim = c(0, 1200))
p_13F_zoom

p_13F_combined <- cowplot::plot_grid(
  p_13F,
  p_13F_zoom,
  ncol = 1,
  labels = c("A", "B")
...
cowplot::save_plot(
  filename = "fig_output/13F_with_error.pdf",
  plot = p_13F_combined,
  base_height = 8,
  base_width = 8
)
...
growth_data_summary <- growth_data |>
  ...
  summarize(
    median_mu.d = median(mu.d, na.rm = TRUE),
...
growth_data_summary |> writexl::write_xlsx("data_output/growth_data.xlsx")
```
...
paste0(
  "Our negative controls exhibit a F13C of ",
  round(neg_ctl_mean, 6),
  "+/- ",
  round(3 * neg_ctl_sd, 6),
  " (3*SD). Therefore, our F13C activity threshold is ",
  round(F13C.activity_threshold, 6),
  " or ",
...
assim_data <- growth_data |>
  ...
  mutate(
    ...
    K1 = calculate_K(a = 1, FT = F13C, Fi = F13C.t0, FL = F13C.label),
    K2 = -(1 / dt) * log(1 - K1),
    # calculate mass of assimilated carbon
    assim.C.fg = calculate_assimilated_C(mass = cell.C.fg, K = K1),
    assim.C.fg2 = calculate_assimilated_C(mass = cell.C.fg, K = K2),
    # calculate rate of assimilation per day
    # fg/day
    rC.fg.d = calculate_assim_rate(mu = assim.C.fg, t = dt),
    rC.fg.d2 = calculate_assim_rate(mu = assim.C.fg2, t = 1),
    rC.fmol.d = rC.fg.d / 12.01,
    rC.fmol.d2 = rC.fg.d2 / 12.01,
  ) |>
  # compare rC values when calculated with K versus K2:
  mutate(rC.fmol.d_residual = rC.fmol.d2 - rC.fmol.d) |>
  ########## NOTE! #############
  # K1 (eq. 3) calculates rC.fmol.d
  # K2 (eq. 4) calculates rC.fmol.d2
  mutate(
    rC.fg.d = case_when(
      rC.fg.d < 0 ~ 0,
      rC.fg.d >= 0 ~ rC.fg.d
    ...
    # EVERYTHING HERE (APPENDED WITH '2') IS CALCULATED FROM K2
    rC.fg.d2 = case_when(
      rC.fg.d2 < 0 ~ 0,
      rC.fg.d2 >= 0 ~ rC.fg.d2
    ),
    rC.fmol.d2 = case_when(
      rC.fmol.d2 < 0 ~ 0,
      rC.fmol.d2 >= 0 ~ rC.fmol.d2
    ),
    assim.C.fg2 = case_when(
      assim.C.fg2 < 0 ~ 0,
      assim.C.fg2 >= 0 ~ assim.C.fg2
    ),
  ) |>
  mutate(
    well = factor(well, levels = c("BA1B", "BA4A", "BA3A")),
  ...
  mutate(
    quantified = if_else(
      F13C - F13C.error > (neg_ctl_mean + (3 * neg_ctl_sd)),
      true = TRUE,
      false = FALSE
    )
  )
...
### compare the different ways of calculating K

```{r}
p_k1_vs_k2 <- assim_data |>
  ggplot(
    aes(
      x = rC.fmol.d,
      y = rC.fmol.d2,
      color = rC.fmol.d_residual
    )
  ) +
  geom_abline(color = "gray", linetype = "dotted") +
  scale_x_log10(
  ...
  scale_y_log10() +
  labs(
    x = latex2exp::TeX("$\\r_C$ (Eq. 3)"),
    y = latex2exp::TeX("$\\r_C$ (Eq. 4)"),
    color = "Residual"
  ) +
  theme_minimal() +
  theme(
...
```{r}
p_k1k2_residual <- assim_data |>
  ggplot(
    aes(
      y = rC.fmol.d_residual,
      x = rC.fmol.d,
      color = rC.fmol.d_residual
    )
  ) +
...
```{r}
# Summarize the assim data
assim_data_median <- assim_data |>
  filter(amendment != "negative control") |>
  filter(rC.fmol.d > 0, quantified) |>
  mutate(
    amendment = factor(
      amendment,
  ...
  arrange(amendment) |>
  group_by(well, amendment) |>
  summarise(median_rC.fmol.d = median(rC.fmol.d, na.rm = TRUE)) |>
  ungroup()

# Plot it!
p_fmolC_k1k2 <- assim_data |>
  filter(amendment != "negative control") |>
  filter(rC.fmol.d > 0) |>
  mutate(
    amendment = factor(
  ...
  pivot_longer(
    cols = c(rC.fmol.d, rC.fmol.d2),
    names_to = "k_type",
    values_to = "rC"
  ) |>
  mutate(
    eq = if_else(k_type == "rC.fmol.d2", true = "Eq. 4", false = "Eq. 3")
  ) |>
  arrange(amendment) |>
  ggplot(
    aes(x = rC, y = depth_str, fill = eq, color = eq)
  ) +
  ggdist::stat_halfeye(
  ...
  labs(
    x = latex2exp::TeX(
      "Cell-specific carbon assimilation rate (fmol C $cell^{-1} \\; day^{-1}$)"
    ),
    y = "",
...
cowplot::plot_grid(
  ...
  cowplot::save_plot(
    filename = "fig_output/K1_K2_plot.pdf",
    base_height = 12,
    base_width = 10
  )
```
...
#### FIG 1: Plot Carbon Assimilation Rate
...
# do it again with the second k calculation method
assim_data_median2 <- assim_data |>
  filter(amendment != "negative control") |>
  filter(rC.fmol.d > 0, quantified) |>
  mutate(
    amendment = factor(
  ...
  arrange(amendment) |>
  group_by(well, depth_str) |>
  summarise(median_rC.fmol.d = median(rC.fmol.d2, na.rm = TRUE)) |>
  ungroup()
...
p_fmolC <- assim_data |>
  ...
  arrange(amendment) |>
  ggplot(
    aes(x = rC.fmol.d, y = amendment)
  ) +
  # plot the median line for each panel using summarized data above
  geom_vline(
    data = assim_data_median,
    aes(xintercept = median_rC.fmol.d),
    linewidth = 0.7
  ) +
...
# Plot it! with the second k calculation method
p_fmolC2 <- assim_data |>
  filter(amendment != "negative control") |>
  filter(rC.fmol.d2 > 0) |>
  mutate(
  ...
  arrange(amendment) |>
  ggplot(
    aes(x = rC.fmol.d2, y = amendment)
  ) +
  # plot the median line for each panel using summarized data above
  geom_vline(
    data = assim_data_median2,
    aes(xintercept = median_rC.fmol.d),
    linewidth = 0.7
  ) +
  # Plot the non-quantified data as red bars
...
test to see to what extent dilution factors affect data
...
assim_data_fmol_summary <- assim_data |>
  ...
  filter(quantified) |>
  summarize(
    median_rC.fmol.d = median(rC.fmol.d2)
  ) |>
  pivot_wider(values_from = median_rC.fmol.d, names_from = amendment) |>
  ungroup()

gt_assim_data_fmol_summary <- assim_data_fmol_summary |>
  gt(rowname_col = "well") |>
  cols_label(depth = "Depth (mbls)") |>
  tab_spanner(columns = -c(depth), label = "Carbon Source") |>
  tab_header(title = "Median Cell-Specific Carbon Assimilation Rates") |>
  tab_footnote(
    footnote = "All values reported in cell-specific fmol C per day.",
...
writexl::write_xlsx(
  ...
  path = "data_output/median_cscar.xlsx"
)
```
...
gtsave(
  gt_fraction_active_summary,
  filename = "data_output/fraction_active.html"
)
```
...
assim_data |>
  ...
  unique() |>
  arrange(well, depth, amendment) |>
  gt()
```

# Summary statistics

Generate the numbers for main text reporting

```{r}
assim_main_text <- assim_data |>
  select(well, depth_str, amendment, rC.fmol.d, quantified) |>
  filter(quantified, amendment != "negative control") |> # not needed for summarizing
  group_by(well, depth_str, amendment) |> # report median + SD for each condition
  summarise(
    rC.fmol.d_median = median(rC.fmol.d, na.rm = TRUE),
    rC.fmol.d_SD = sd(rC.fmol.d, na.rm = TRUE)
  )

glimpse(assim_main_text)

# give nice table for BA3A
assim_main_text |>
  filter(well == "BA3A") |>
  arrange(rC.fmol.d_median) |>
  knitr::kable()

# give nice table, ordered
assim_main_text |> arrange(rC.fmol.d_median, amendment) |> knitr::kable()
```
...
paste(
  "A total of",
  n_cells,
  "individual microbial cells were analyzed by nanoSIMS in this study."
)
```
...
`ANOVA (Type II) F-statistic: 763.41 on 2 and 6298 DF, p-value: < 2.2e-16`.
...
`ANOVA (Type II) F-statistic: 417.56 on 2 and 6296 DF, p-value < 2.2e-16`.
...
carbon_posthoc <- assim_data_anova |>
  mutate(
    amendment = factor(
      amendment,
      levels = c("bicarbonate", "acetate", "formate")
  ...
  nest() |>
  mutate(
    lm_fit = map(data, \(d) lm(rC.fmol.d ~ amendment, data = d)),
    emmeans = map(lm_fit, \(m) emmeans(m, ~amendment)),
    contrasts = map(emmeans, \(e) pairs(e, adjust = "tukey"))
  )

# Unnest into one tidy table
carbon_posthoc_tidy <- carbon_posthoc |>
  mutate(tidy_contrasts = map(contrasts, tidy)) |>
  select(well, depth_str, tidy_contrasts) |>
...
writexl::write_xlsx(
  carbon_posthoc_tidy,
  path = "data_output/carbon_posthoc_test.xlsx"
)
```
...
`F-statistic: 677.86 on 2 and 2010 DF, p-value: < 2.2e-16`
...
assim_data |> writexl::write_xlsx("data_output/assim_data.xlsx")
```