...
voa_corrected_ctl |> 
  ...
  dplyr::rename(Acetate_uM = Acetate,
         Formate_uM = Formate) |> 
  gt() |> 
...
p_13F_zoom <- p_13F %+% coord_cartesian(ylim = c(0.008, 0.015), xlim = c(0, 1200))
p_13F_zoom
...
p_fmolC_k1k2 <- assim_data |> 
  ...
  mutate(eq = if_else(k_type == "rC.fmol.d2", true = "Eq. 4", false = "Eq. 3")) |> 
  arrange(amendment) |> 
  ggplot(
    aes(x = rC, y = depth_str,
        fill = eq, color = eq)
  ) +
  ggdist::stat_halfeye(
  ...
  labs(
    x = latex2exp::TeX("Cell-specific carbon assimilation rate (fmol C $cell^{-1} \\; day^{-1}$)"),
    y = "",
...
```{r}
assim_data |> 
  select(well, depth, amendment, dt) |> unique() |> arrange(well, depth, amendment) |> gt()
```
