...
mu_star_gt <- assim_data_summary |> 
  ...
  ungroup() |>
  pivot_wider(names_from = c(amendment), values_from = c(mu_star_median, fgC_biomass_median, MSP_WgC_median)) |> 
  gt::gt() |> 
  gt::cols_label(
    well = "Borehole",
    depth = "Depth (mbls)",
  ...
  gt::tab_spanner(
    columns = c(mu_star_median_bicarbonate, mu_star_median_acetate, 
                mu_star_median_formate), 
    label = "µ* {{(yr^-1 )}}"
    ) |> 
  gt::tab_spanner(
    columns = c(fgC_biomass_median_bicarbonate, fgC_biomass_median_acetate, 
                fgC_biomass_median_formate), 
    label = "Cell-specific biomass (fg C)"
    ) |> 
  gt::tab_spanner(
    columns = c(MSP_WgC_median_bicarbonate, MSP_WgC_median_acetate, 
                MSP_WgC_median_formate), 
    label = "MSP {{(W g C^-1 )}}"
    ) |> 
  gt::data_color(columns = c(fgC_biomass_median_bicarbonate,
                             fgC_biomass_median_acetate, 
                             fgC_biomass_median_formate), 
                 palette = "Blues") |> 
  gt::data_color(columns = c(MSP_WgC_median_bicarbonate, 
                             MSP_WgC_median_acetate, 
                             MSP_WgC_median_formate), 
                 palette = "Greens") |> 
  gt::data_color(columns = c(mu_star_median_bicarbonate, 
                             mu_star_median_acetate, 
                             mu_star_median_formate), 
                 palette = "Reds") |> 
  gt::fmt_scientific(columns = -c(well, depth)) |> 
  gt::fmt_number(decimals = 1, columns = c(fgC_biomass_median_bicarbonate, fgC_biomass_median_acetate, fgC_biomass_median_formate))
...
# cowplot::save_plot(plot = p_h2_ch4, filename = "fig_output/h2_ox_tonnes_ADJ.pdf",
...
gt_landscape <- assim_metabolic |> 
  ...
  select(well, depth_str, median_h2_ox, min_h2_ox, 
         max_h2_ox, median_ch4_prod, min_ch4_prod, max_ch4_prod) |>
