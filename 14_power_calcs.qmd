...
mu_star_gt <- assim_data_summary |> 
  filter(mu_star.yr > 0) |> 
  select(well, depth, amendment, mu_star.yr) |> 
  group_by(well, depth, amendment) |> 
  dplyr::summarise(
    mu_star_median = median(mu_star.yr)
  ) |> 
  ungroup() |> 
  pivot_wider(names_from = c(amendment), values_from = mu_star_median) |> 
  gt::gt() |> 
  gt::cols_label(
    well = "Borehole",
    depth = "Depth (mbs)"
  ) |> 
  gt::tab_spanner(columns = c(bicarbonate, acetate, formate), label = "µ* {{(yr^-1 )}}") |> 
  gt::data_color(columns = -c(well, depth), palette = "Blues") |> 
  gt::fmt_scientific(columns = c(bicarbonate, acetate, formate))
  
mu_star_gt

gt::gtsave(data = mu_star_gt, filename = "data_output/mu_star.html")
```
