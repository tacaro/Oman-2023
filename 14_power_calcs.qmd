...
Following a similar approach for methanogenesis, [@hoehler2001] predict that methanogeneic archaea are supported by energy yields as small as -10.6 kJ/mol CH4 (-1060 J / mol H2)
...
p_h2 <- assim_metabolic |> 
  ...
  geom_vline(xintercept = 3.95, color = "red", linewidth = 0.5) +
  ggdist::stat_pointinterval(point_interval = "mean_qi", color = "black") +
...
cowplot::save_plot(plot = p_MSP_dens_combined,
                   ...
                   filename = "fig_output/MSP_literature_density.pdf"
                   )
```
...
mean_MSP <- assim_metabolic |> 
  group_by(well, depth) |> 
  summarize(
    all = mean(MSP.J_s_g, na.rm = TRUE)
  )

mean_MSP_C <- assim_metabolic |> 
  group_by(well, depth, amendment) |> 
  summarize(mean_MSP_C = mean(MSP.J_s_g, na.rm = TRUE)) |> 
  pivot_wider(names_from ="amendment", values_from = "mean_MSP_C") |> 
  left_join(mean_MSP, by = c("well", "depth")) |> 
  ungroup()

gt_MSP <- mean_MSP_C |> 
  gt::gt(groupname_col = "well") |> 
  gt::cols_label(all = "mean") |> 
...
mean_MSP_C |> writexl::write_xlsx("data_output/MSP.xlsx")
```
