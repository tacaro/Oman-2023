...
gt_landscape <- assim_metabolic |> 
  ...
  summarise(
    mean_h2_ox = mean(MA_H2ox.tonnes_yr_km3_ADJ, na.rm = TRUE),
    mean_ch4_prod = mean(MA_CH4.tonnes_yr_km3_ADJ, na.rm = TRUE),
  ...
  select(well, depth_str, mean_h2_ox, min_h2_ox, max_h2_ox, mean_ch4_prod, min_ch4_prod, max_ch4_prod) |>
  arrange(desc(depth_str)) |> 
  gt::gt() |> 
  gt::tab_spanner(
    label = "{{CH_4 Produced tonnes km^-1 yr^-1}}",
    columns = c(mean_ch4_prod, min_ch4_prod, max_ch4_prod)
  ) |> 
  gt::tab_spanner(
    label = "{{H_2 tonnes km^-1 yr^-1}}",
    columns = c(mean_h2_ox, min_h2_ox, max_h2_ox)
  ) |> 
...
assim_metabolic |> 
  ...
  group_by(well, depth_str) |> 
  summarise(
    mean_h2_ox = mean(MA_H2ox.tonnes_yr_km3, na.rm = TRUE),
    mean_ch4_prod = mean(MA_CH4.tonnes_yr_km3, na.rm = TRUE),
