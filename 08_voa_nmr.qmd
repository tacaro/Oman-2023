...
well_palette <- c(well_palette, "black")

p_voa <- voa_corrected_ctl |> 
  mutate(well = factor(well, levels = c("BA1B", "BA4A", "BA3A", "control"))) |> 
  mutate(sample_id = fct_reorder(sample_id, as.numeric(well))) |> 
  ggplot(
    aes(
      x = sample_id,
      y = uM_corr,
...
cowplot::save_plot(
  plot = p_voa,
  filename = "fig_output/voa_data.pdf",
...
voa_corrected_ctl |> 
  mutate(well = factor(well, levels = c("BA1B", "BA4A", "BA3A", "control"))) |> 
  mutate(sample_id = fct_reorder(sample_id, as.numeric(well))) |> 
  writexl::write_xlsx(path = "data_output/voa_data.xlsx")
