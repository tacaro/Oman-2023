...
> Note: this analysis script is for the second set of 16S data

From `09b_16S_Data_Reduction_secondset.qmd`, we have output caches and text files:
...
```{r}
taxtable <- read_rds("data/16S_batch2/outputs/03_tabletax/tax_final.rds") |> 
  # convert to tibble
  as_tibble() |> 
  # fix column headers
...
seq_tab <- read_delim("data/16S_batch2/outputs/03_tabletax/seqtab_final.txt") |> 
...
writexl::write_xlsx(metadata, path = "data/sample_metadata_16S_batch2.xlsx")
...
The library sizes of the positive samples range from low (hundreds of reads) to 60000 reads. Overall, the blanks look really good: low reads in blanks and high reads in samples. We shall proceed.
...
write_rds(ps, "cache/ps_batch2.RDS")

# save decontam'd PS object
write_rds(ps.decontam, "cache/ps-decontam_batch2.RDS")

# save the contaminant PS object
write_rds(ps.contaminants, "cache/ps-contaminants_batch2.RDS")

# save the melted phyloseq object for tidy analyses (less RAM efficient!)
writexl::write_xlsx(ps.decontam |> ps_melt(), path = "data_output/ps_decontam_batch2.xlsx")
```
...
```{r}
ps.reads <- ps.decontam |> ps_melt()

reads <- ps.reads |> 
  group_by(Sample, well, depth, amendment) |> 
  summarise(
    nreads = sum(Abundance, na.rm = TRUE)
  )
...
p_13C_relabund <- ps.13C_relabund |> 
  #tax_agg("Genus") |> 
...
p_all <- ps.decontam |> 
  ...
  facet_grid(
    rows = vars(experiment, well, depth),
    scales = "free",
    space = "free"
  ) +
  coord_flip() +
