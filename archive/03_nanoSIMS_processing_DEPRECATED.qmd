...
p_ratio %+% coord_cartesian(xlim = c(0, 0.05)) %+% labs(caption = "Zoomed in")
```
...
Before our work with EMSL, we used the following estimates of OA concentrations from previous studies nearby wells, taking the mean values.
 ...
 mutate(
   ...
   mutate(
     mu.d = calculate_mu(
       ...
       a = 1,
       F2 = at13C, # measured cell F13C (fractional abundance, not at. %)
...
p_13F_zoom <- p_13F %+% coord_cartesian(xlim = c(0, 0.1))
p_13F_zoom

cowplot::save_plot(plot = p_13F_zoom, filename = "fig_output/p13F_zoom.pdf", base_height = 8, base_width = 12)
```
...
p_fmolC <- assim_data |> 
  ...
  filter(rC.fmol.d > 0) |> 
  ggplot(
    aes(x = rC.fmol.d, y = depth_str, color = amendment)
  ) +
  geom_point(
    data = function(df) df |> filter(!quantified),
    shape = 124,
  ...
  ggdist::stat_halfeye(
    color = "black", 
    height = 0.8,
    point_interval = "mean_qi", # mean quantile interval
...
cowplot::save_plot(
  plot = p_fmolC,
  filename = "fig_output/p_fmolC.pdf",
  base_height = 6,
  base_width = 10
)
```
...
gt_assim_data_fmol_summary <- assim_data_fmol_summary |> 
  ...
  gt::tab_header(title = "Mean Cell-Specific Carbon Assimilation Rates") |> 
...
gt::gtsave(gt_assim_data_fmol_summary, filename = "data_output/mean_cscar.html")
writexl::write_xlsx(assim_data_fmol_summary, path = "data_output/mean_cscar.xlsx")
...
gt::gtsave(gt_fraction_active_summary, filename = "data_output/fraction_active.html")
