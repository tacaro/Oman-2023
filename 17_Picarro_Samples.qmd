...
crds_moles <- crds_diff |> 
  ...
  mutate(
    ...
    umol_per_day_SIP = umol_total_SIP / as.numeric(dt),
    umol_per_day_SIP.upper = umol_total_SIP.upper / as.numeric(dt),
    umol_per_day_SIP.lower = umol_total_SIP.lower / as.numeric(dt)
  )

gt_crds_moles <- crds_moles |> 
  select(well, depth, amendment, percent_ch4_from_label, 
         umol_total, umol_total_SIP, umol_per_day, umol_per_day_SIP) |> 
  mutate(well = factor(well, levels = c("BA1B", "BA4A", "BA3A"))) |> 
  ...
  tab_spanner(
    ...
    columns = c(umol_per_day, umol_per_day_SIP)
  ) |> 
  tab_spanner(
  ...
  fmt_percent(columns = c(percent_ch4_from_label),
              scale_values = FALSE, decimals = 1) |> 
  fmt_number(columns = c(umol_total, umol_total_SIP, umol_per_day, umol_per_day_SIP)
             ,decimals = 1) |> 
