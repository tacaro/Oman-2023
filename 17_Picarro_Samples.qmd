...
crds_diff <- crds_w_fl |> 
  mutate(
    ...
    percent_ch4_from_label_upper = ((HR_at13_CH4_mean + HR_at13_CH4_sd) / at13C_label) * 100,
    percent_ch4_from_label_lower = ((HR_at13_CH4_mean - HR_at13_CH4_sd) / at13C_label) * 100,
    ) |> 
  mutate(
  ...
  mutate(
    depth_str = factor(depth_str, levels = c(
      "Shallow (20m)", "Intermediate (150m)", "Deep (250/270m)"
      )
      )
  )
...
p_crds_diff3 <- crds_diff |> 
  ...
  mutate(
    amendment = factor(amendment, levels = c("formate", "acetate", "bicarbonate")),
...
p_crds_diff_new <- crds_diff |> 
  mutate(
    depth_str = fct_rev(depth_str),
    amendment = factor(amendment, levels = c("bicarbonate", "formate", "acetate")),
    well_str = case_when(well == "BA1B" ~ "Mildly Alkaline \n(BA1B)",
                         well == "BA4A" ~ "Moderately Alkaline \n(BA4A)",
                         well == "BA3A" ~ "Hyperalkaline \n(BA3A)"),
    well_str = factor(well_str, 
                      levels = c("Mildly Alkaline \n(BA1B)", 
                                 "Moderately Alkaline \n(BA4A)", 
                                 "Hyperalkaline \n(BA3A)"),
    )
    ) |>
