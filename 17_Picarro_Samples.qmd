...
if (!cache_exists) {
  ...
  crds_data <- bind_rows(
    ...
    mutate(datetime = with_tz(datetime, "MST"), datetime = datetime + hours(1))
...
if (!cache_exists) {
  sample_map <- readxl::read_excel(
    "data/Picarro/OM23_Picarro_Data.xlsx",
    sheet = 1
  ) |>
    as_tibble() |>
    mutate(
...
crds_data_mapped |>
  ...
  scale_x_datetime(
    limits = c(
      as.POSIXct("2024-03-19 09:00:00", tz = "MST"),
...
crds_data_mapped |>
  ...
  scale_x_datetime(
    limits = c(
      as.POSIXct("2024-03-18 09:00:00", tz = "MST"),
...
crds_data_mapped |>
  ...
  scale_x_datetime(
    limits = c(
      as.POSIXct("2024-03-20 09:00:00", tz = "MST"),
...
crds_summary <- crds_data_mapped |>
  ...
  select(
    ...
    HR_12CH4_mean,
    HR_13CH4_mean,
    HR_Delta_iCH4_Raw_mean,
    HR_12CH4_sd,
    HR_13CH4_sd,
    HR_Delta_iCH4_Raw_sd
  ) |>
  mutate(
    # Means:
...
labels <- readxl::read_excel(
  "data_output/effective_label_concentrations.xlsx"
) |>
  select(
    well,
    depth,
    effective_at13C_formate,
    effective_at13C_bicarbonate,
    effective_at13C_acetate
  ) |>
...
p_error <- crds_w_fl |>
  ...
  theme_bw() +
  theme(aspect.ratio = 1, plot.margin = margin(0.25, 0.25, 0.25, 0.25, "cm"))
p_error
...
crds_diff <- crds_w_fl |>
  mutate(
    ...
    percent_ch4_from_label_upper = ((HR_at13_CH4_mean + HR_at13_CH4_sd) /
      at13C_label) *
      100,
    percent_ch4_from_label_lower = ((HR_at13_CH4_mean - HR_at13_CH4_sd) /
      at13C_label) *
      100,
  ) |>
  mutate(
    upper_diff = percent_ch4_from_label_upper - percent_ch4_from_label,
    lower_diff = percent_ch4_from_label - percent_ch4_from_label_lower
  ) |>
  mutate(
    ...
    percent_ch4_from_label_upper = percent_ch4_from_label + upper_diff,
    percent_ch4_from_label_lower = percent_ch4_from_label - lower_diff
  ) |>
  mutate(
    percent_ch4_from_other = ((at13C_label - HR_at13_CH4_mean) / at13C_label) *
      100
  ) |>
  mutate(
...
p_crds_diff3 <- crds_diff |>
  ...
  mutate(
    type = case_when(
      type == "percent_ch4_from_label" ~ "13C Tracer",
    ...
    source = factor(
      source,
      levels = c("12C-Other", "13C-bicarbonate", "13C-formate", "13C-acetate")
    )
  ) |>
  mutate(
  ...
  mutate(
    amendment = factor(
      amendment,
      levels = c("formate", "acetate", "bicarbonate")
    ),
    condition = fct_reorder(condition, as.numeric(amendment))
  ) |>
  ggplot() +
  aes(
    x = fraction,
...
```{r}
# crds_diff |>
#   select(well, depth_str, amendment, HR_at13_CH4_mean, HR_at13_CH4_sd, percent_ch4_from_label, percent_ch4_from_label_lower, percent_ch4_from_label_upper, at13C_label) |> view()

p_crds_diff_new <- crds_diff |>
  mutate(
    depth_str = fct_rev(depth_str),
    amendment = factor(
      amendment,
      levels = c("bicarbonate", "formate", "acetate")
    ),
    well_str = case_when(
      well == "BA1B" ~ "Mildly Alkaline \n(BA1B)",
      well == "BA4A" ~ "Moderately Alkaline \n(BA4A)",
      well == "BA3A" ~ "Hyperalkaline \n(BA3A)"
    ),
    well_str = factor(
      well_str,
      levels = c(
  ...
  mutate(
    detected = case_when(
      well == "BA3A" & depth == 150 & amendment == "bicarbonate" ~ FALSE,
      well == "BA3A" & depth == 270 & amendment == "bicarbonate" ~ FALSE,
      well == "BA3A" & depth == 150 & amendment == "formate" ~ FALSE,
      well == "BA3A" & depth == 270 & amendment == "formate" ~ FALSE,
      well == "BA3A" & depth == 150 & amendment == "acetate" ~ FALSE,
      well == "BA3A" & depth == 270 & amendment == "acetate" ~ FALSE,
      TRUE ~ TRUE
    ),
    depth_str = fct_rev(depth_str),
    amendment = fct_rev(factor(
      amendment,
  ...
  geom_pointrange(
    ...
    aes(
      fill = well,
      xmin = percent_ch4_from_label_lower,
      xmax = percent_ch4_from_label_upper
    ),
    size = 1
  ) +
  ...
  scale_fill_brewer() +
  facet_grid(depth_str ~ well_str) +
  theme_minimal() +
  labs(
    x = latex2exp::TeX("Fraction $CH_4$ from $^{13}C$ Probe"),
    y = "",
    fill = ""
  ) +
  guides(
    shape = "none"
    #fill = guide_colorbar(barheight = 0.5, barwidth = 10, unit = "cm")
  ) +
  theme(
...
ggsave(
  filename = "fig_output/ch4_crds_updated.pdf",
  plot = p_crds_diff_new,
  dpi = 300,
  height = 5,
  width = 9
)
...
crds_diff |>
  select(
    ...
    depth,
    amendment,
    percent_ch4_from_label,
    percent_ch4_from_other
  ) |>
  rename(
    `CH4 From 13C-Probe` = percent_ch4_from_label,
  ...
  fmt_percent(
    columns = c(`CH4 From Other`, `CH4 From 13C-Probe`),
    scale_values = FALSE,
    decimals = 1
  ) |>
  ...
  tab_style(
    style = list(cell_fill(color = "#da3b26"), cell_text(color = "white")),
    locations = cells_row_groups(groups = "BA1B")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#479ff8"), cell_text(color = "white")),
    locations = cells_row_groups(groups = "BA4A")
  ) |>
  tab_style(
    style = list(cell_fill(color = "#004d01"), cell_text(color = "white")),
...
crds_moles <- crds_diff |>
  ...
  mutate(
    ...
    umol_per_ml_per_day_SIP.upper = (umol_total_SIP.upper / 50) /
      as.numeric(dt),
