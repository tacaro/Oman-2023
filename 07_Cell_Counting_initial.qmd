...
cell_counts_summarized <- cell_counts |>
  ...
  group_by(site) |>
  summarize(count.sum = sum(count), count.sd = sd(count)) |>
  mutate(n.FOV = 3)
```
...
p_cell_concs <- cell_concs |>
  group_by(well) |>
  mutate(
    site = fct_reorder(.f = site, .x = depth),
...
p_corr <- assim_data_cells |>
  ...
  geom_point(aes(color = well, shape = as.factor(depth)), size = 3) +
...
cowplot::save_plot(
  plot = p_combined,
  filename = "fig_output/correlations_w_cscar.pdf",
  base_height = 6,
  base_width = 12
)
```
